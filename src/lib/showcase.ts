 ...
 * shelves, plus the integrations band. Editorial content is hand-picked and
 ...
 *
 * `slug` must exist in the template registry. Links go to /templates/<slug>.
...
export interface ShowcaseTemplate {
  ...
  from: string;
  to: string;
  /** Logo basenames under /public/logos, only where the template reads that source. */
  logos: string[];
